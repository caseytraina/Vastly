        ...
        ZStack {
            ...
            VStack {
                ...
                if processed {
                    ...
                    ScrollView {
                        ForEach(authModel.liked_videos.indices) { i in
                            
                            NavigationLink(destination: {
                                SingleVideoView(video: videos[i])
                                    .environmentObject(authModel)
                                    .environmentObject(viewModel)

                            }, label: {
                                HStack {
                                    AsyncImage(url: getThumbnail(video: self.videos[i])) { mainImage in
                                        mainImage.resizable()
                                            .frame(width: screenSize.width * 0.18, height: screenSize.width * 0.18)
                                    } placeholder: {
                                        
                                        AsyncImage(url: self.videos[i].author.fileName) { image in
                                            image.resizable()
                                                .frame(width: screenSize.width * 0.18, height: screenSize.width * 0.18)
                                        } placeholder: {
                                            ZStack {
                                    ...
                                    VStack(alignment: .leading) {
                                        MyText(text: "\(videos[i].title)", size: screenSize.width * 0.035, bold: true, alignment: .leading, color: .white)
                                            .lineLimit(2)
                                        MyText(text: "\(videos[i].author.name ?? "")", size: screenSize.width * 0.035, bold: false, alignment: .leading, color: Color("AccentGray"))
                                            .lineLimit(1)
                                    }
                                    Spacer()
...
//                self.isPlaying = false
    ...
    private func getVideos() {
        for title in authModel.liked_videos {
            let allVideos = Array(viewModel.videos.values.flatMap {$0})
            
            let video: Video = allVideos.first(where: {$0.title == title} ) ?? EMPTY_VIDEO
